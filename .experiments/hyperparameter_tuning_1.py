...
parser.add_argument('--test_type', type=str, choices=[
    'lookback',
    'BS',
    'lr',
    'model_dim',
...
if __name__ == "__main__":
    ...
    # make folder
    os.makedirs(os.path.join(args.savedir,f'ray_{generate_random_string(args.filename_seed)}_lookahead_{args.lookahead}_type_{args.test_type}'), exist_ok=True)
    ...
    tuner = tune.Tuner(
        ...
        run_config=train.RunConfig(
            name="pbt_test",
            stop={"mape": 2.5},
            storage_path=os.path.join(args.savedir,f'ray_{generate_random_string(args.filename_seed)}_lookahead_{args.lookahead}_type_{args.test_type}'),
        ...
        param_space={
            "lookback": tune.grid_search([12, 24, 48, 96, 168, 192]) if args.test_type=='lookback' else 12,
            "BS": tune.grid_search([16, 32, 64, 128]) if args.test_type=='BS' else 64,
            "lr": tune.grid_search([1e-5, 5e-5, 1e-4, 1e-3]) if args.test_type=='lr' else 5e-5,
            "model_dim": tune.grid_search([4, 8, 16, 32, 64, 128, 256, 512]) if args.test_type=='model_dim' else 64,
            "layers": tune.grid_search([2, 3, 4, 5, 6, 7]) if args.test_type=='layers' else 4
        },
    )
