...
class DARNNBase(nn.Module):
    ...
    def to(self, device):
        ...
        return super(DARNNBase,self).to(device)
...
class DARNN(nn.Module):
    ...
    def __init__(
        ...
        # DARNN parameters
        input_size = x_size + y_size + u_size + s_size
        seq_len = lookback
        self.y_size, self.x_size, self.s_size, self.u_size = y_size, x_size, s_size, u_size
        self.lookback, self.lookahead = lookback, lookahead
        
        # DARNN
        self.darnn = DARNNBase(
            input_size,
            hidden_size,
            seq_len,
            num_lstm_layers,
    ...
    def forward(self, x):
        ...
        return out[:,None,:].repeat(1,self.lookahead,1)