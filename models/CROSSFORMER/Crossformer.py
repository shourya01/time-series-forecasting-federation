...
class DSW_embedding(nn.Module):
    def __init__(self, seg_len, d_model, dtype=torch.float32):
        ...
        self.linear = nn.Linear(seg_len, d_model, dtype=dtype)
...
class SegMerging(nn.Module):
    ...
    '''
    def __init__(self, d_model, win_size, norm_layer=nn.LayerNorm, dtype=torch.float32):
        super().__init__()
        self.d_model = d_model
        self.win_size = win_size
        self.linear_trans = nn.Linear(win_size * d_model, d_model, dtype=dtype)
        self.norm = norm_layer(win_size * d_model, dtype=dtype)
...
class AttentionLayer(nn.Module):
    ...
    '''
    def __init__(self, d_model, n_heads, d_keys=None, d_values=None, dropout = 0.1, dtype=torch.float32):
...
class TwoStageAttentionLayer(nn.Module):
    ...
    '''
    def __init__(self, seg_num, factor, d_model, n_heads, d_ff = None, dropout=0.1, dtype=torch.float32):
        super(TwoStageAttentionLayer, self).__init__()
        d_ff = d_ff or 4*d_model
        self.time_attention = AttentionLayer(d_model, n_heads, dropout = dropout, dtype=dtype)
        self.dim_sender = AttentionLayer(d_model, n_heads, dropout = dropout, dtype=dtype)
        self.dim_receiver = AttentionLayer(d_model, n_heads, dropout = dropout, dtype=dtype)
        self.router = nn.Parameter(torch.randn(seg_num, factor, d_model, dtype=dtype))
        ...
        self.norm4 = nn.LayerNorm(d_model, dtype=dtype)

        self.MLP1 = nn.Sequential(nn.Linear(d_model, d_ff, dtype=dtype),
                                nn.GELU(),
                                nn.Linear(d_ff, d_model, dtype=dtype))
        self.MLP2 = nn.Sequential(nn.Linear(d_model, d_ff, dtype=dtype),
                                nn.GELU(),
                                nn.Linear(d_ff, d_model, dtype=dtype))
...
class scale_block(nn.Module):
    ...
    def __init__(self, win_size, d_model, n_heads, d_ff, depth, dropout, \
                    seg_num = 10, factor=10, dtype=torch.float32):
        super(scale_block, self).__init__()

        if (win_size > 1):
            self.merge_layer = SegMerging(d_model, win_size, nn.LayerNorm, dtype=dtype)
        else:
...
class DecoderLayer(nn.Module):
    ...
    '''
    def __init__(self, seg_len, d_model, n_heads, d_ff=None, dropout=0.1, out_seg_num = 10, factor = 10, dtype=torch.float32):
        super(DecoderLayer, self).__init__()
        self.self_attention = TwoStageAttentionLayer(out_seg_num, factor, d_model, n_heads, \
                                d_ff, dropout, dtype=dtype)    
        self.cross_attention = AttentionLayer(d_model, n_heads, dropout = dropout, dtype=dtype)
        self.norm1 = nn.LayerNorm(d_model, dtype=dtype)
        self.norm2 = nn.LayerNorm(d_model, dtype=dtype)
        self.dropout = nn.Dropout(dropout)
        self.MLP1 = nn.Sequential(nn.Linear(d_model, d_model, dtype=dtype),
                                nn.GELU(),
                                nn.Linear(d_model, d_model, dtype=dtype))
        self.linear_pred = nn.Linear(d_model, seg_len, dtype=dtype)
...
class Decoder(nn.Module):
    ...
    def __init__(self, seg_len, d_layers, d_model, n_heads, d_ff, dropout,\
                router=False, out_seg_num = 10, factor=10, dtype=torch.float32):
        ...
        for i in range(d_layers):
            self.decode_layers.append(DecoderLayer(seg_len, d_model, n_heads, d_ff, dropout, \
                                        out_seg_num, factor, dtype=dtype))
...
class CrossformerBase(nn.Module):
    def __init__(self, data_dim, in_len, out_len, seg_len, win_size = 4,
        ...
        # Embedding
        self.enc_value_embedding = DSW_embedding(seg_len, d_model, dtype=dtype)
        self.enc_pos_embedding = nn.Parameter(torch.randn(1, data_dim, (self.pad_in_len // seg_len), d_model, dtype=dtype))
        self.pre_norm = nn.LayerNorm(d_model, dtype=dtype)

        # Encoder
        self.encoder = Encoder(e_layers, win_size, d_model, n_heads, d_ff, block_depth = 1, \
                                    dropout = dropout,in_seg_num = (self.pad_in_len // seg_len), factor = factor, dtype=dtype)
        
        # Decoder
        self.dec_pos_embedding = nn.Parameter(torch.randn(1, data_dim, (self.pad_out_len // seg_len), d_model, dtype=dtype))
        self.decoder = Decoder(seg_len, e_layers + 1, d_model, n_heads, d_ff, dropout, \
                                    out_seg_num = (self.pad_out_len // seg_len), factor = factor, dtype=dtype)
...
class Crossformer(nn.Module):
    ...
    def __init__(
        ...
        if in_len <= 4:
            raise ValueError("Crossformer doesnt allow input sequence lengths less than 5!")
        else:
            f = lambda x: x // win_size if x % win_size != 0 else (x // win_size) - 1
            seg_len = f(in_len)
            
        self.crossformer = CrossformerBase(
            data_dim = enc_in,
            in_len = in_len,
            out_len = out_len,
            seg_len = seg_len,
